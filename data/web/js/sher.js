...
sher.show_error_message = function(errors, ele) {
...
sher.show_ok_message = function(msg, ele) {  	
	$('<div/>')
			...
			$(this).ajaxSubmit({
				...
				success: function(data){
					...
					if(data.is_error){
						...
						sher.show_error_message(data.message, event.target);
					}else{
...
// hook 评论行为
sher.hook_comment_page = function(){
	$('#comment-form').form({
		content: {
			identifier  : 'content',
			rules: [
...
// 处理批量附件
sher.rebuild_batch_assets = function(id){
